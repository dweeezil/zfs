...
{
	if is_linux; then
		typeset THREAD_PID=$(pgrep spa_vdev_remove)
		[[ "$THREAD_PID" ]] ||
...
function wait_for_thread_running # spa_address
{
	if is_linux; then
		SECONDS=0
	...
	log_must ensure_thread_running $1
...
#
log_must dd if=/dev/urandom of=$TESTDIR/$TESTFILE0 bs=64M count=8
...
#
log_must zinject -d $REMOVEDISK -D 25:1 $TESTPOOL
...
#
# Wait briefly for the thread to be started.
#
wait_for_thread_running $SPA_ADDR
